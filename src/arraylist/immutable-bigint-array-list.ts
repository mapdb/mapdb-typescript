...
import type { MapDbList } from "../api/index.js";
import { BigIntArrayList } from "./bigint-array-list.js";

/**
 * Immutable array list holding bigint values.
 * Wraps a mutable BigIntArrayList, exposing only read-only operations.
 */
export class ImmutableBigIntArrayList implements MapDbList<bigint> {
  private readonly delegate: BigIntArrayList;

  constructor(source: BigIntArrayList) {
    // Copy all entries into a fresh mutable list so the caller cannot mutate our data.
    this.delegate = new BigIntArrayList(source.size());
  ...
  static of(values: bigint[]): ImmutableBigIntArrayList {
    return new ImmutableBigIntArrayList(BigIntArrayList.of(values));
  }
  ...
  *[Symbol.iterator](): IterableIterator<bigint> {
  ...
  select(predicate: (value: bigint) => boolean): ImmutableBigIntArrayList {
    return new ImmutableBigIntArrayList(this.delegate.select(predicate));
  }

  /** Returns a new immutable list with elements NOT satisfying the predicate. */
  reject(predicate: (value: bigint) => boolean): ImmutableBigIntArrayList {
    return new ImmutableBigIntArrayList(this.delegate.reject(predicate));
  }
  ...
  toMutable(): BigIntArrayList {
    return BigIntArrayList.of(this.delegate.toArray());
  }
