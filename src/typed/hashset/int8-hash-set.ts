...
/**
 * Hash set backed by Int8Array with Uint8Array occupied bitmap.
 * Elements: 1 bytes each, contiguous memory, no GC pressure.
 * Open-addressing with linear probing.
 * Memory: 2 bytes/slot (vs ~50-70 bytes in Set<number>).
 */
export class Int8HashSet {
  ...
  union(other: Int8HashSet): Int8HashSet {
    const result = new Int8HashSet(this._size + other._size);
  ...
  *values(): Generator<number> {
  ...
  static fromImmutable(imm: {
    ...
    const set = new Int8HashSet(arr.length);
    for (const v of arr) {
      set.add(v);
    }
    return set;
