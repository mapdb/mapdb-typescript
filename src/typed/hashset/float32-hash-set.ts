...
export class Float32HashSet {
  ...
  constructor(capacity = DEFAULT_CAPACITY) {
    ...
    this.items = new Float32Array(this.capacity);
  ...
  static bulkLoadExact(
    ...
    const set = new Float32HashSet(hashCapacityFor(n));
   ...
   */
  static bulkLoad(values: Iterable<number>, opts?: BulkLoadOptions): Float32HashSet {
    const onDuplicate = opts?.onDuplicate ?? "error";
    ...
    const set =
      hint !== undefined ? new Float32HashSet(hashCapacityFor(hint)) : new Float32HashSet();
    let i = 0;
    for (const value of values) {
  ...
  union(other: Float32HashSet): Float32HashSet {
    const result = new Float32HashSet(this._size + other._size);
  ...
  intersect(other: Float32HashSet): Float32HashSet {
  ...
  difference(other: Float32HashSet): Float32HashSet {
    const result = new Float32HashSet();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !other.has(this.items[i])) {
        result.add(this.items[i]);
