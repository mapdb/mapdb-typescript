...
import { ImmutableInt16HashSet } from "./immutable_int16-hash-set.js";
import { Int16HashSet } from "./int16-hash-set.js";

describe("ImmutableInt16HashSet generated", () => {
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new Int16HashSet();
    mutable.add(1);
    mutable.add(2);
    const imm = ImmutableInt16HashSet.fromMutable(mutable);
    mutable.add(3);
  ...
  it("size and isEmpty", () => {
    const empty = ImmutableInt16HashSet.of([]);
    expect(empty.size()).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableInt16HashSet.of([1]);
    expect(nonEmpty.size()).toBe(1);
    expect(nonEmpty.isEmpty()).toBe(false);
  });

  it("select returns MUTABLE", () => {
    const set = ImmutableInt16HashSet.of([1, 2, 3, 4, 5]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableInt16HashSet.of([1, 2, 3]);
  ...
  it("filter", () => {
    const set = ImmutableInt16HashSet.of([1, 2, 3, 4]);
  ...
  it("memoryBytes", () => {
    ...
    expect(set.memoryBytes()).toBeGreaterThanOrEqual(3 * (2 + 1));
  });
  ...
  it("has no add method", () => {
    const set = ImmutableInt16HashSet.of([1]);
