...
/**
 * Immutable hash set backed by Float64Array with Uint8Array occupied bitmap.
 ...
 */
export class ImmutableFloat64HashSet {
  ...
  static of(values: number[]): ImmutableFloat64HashSet {
    const set = new Float64HashSet(values.length);
    for (const v of values) {
      set.add(v);
    }
    return ImmutableFloat64HashSet.fromMutable(set);
  }

  /** Creates an immutable copy from a mutable set (defensive copy). */
  static fromMutable(mutable: Float64HashSet): ImmutableFloat64HashSet {
    const arr = mutable.toArray();
    const c = nextPowerOfTwo(Math.max(arr.length * 2, 16));
    const items = new Float64Array(c);
    ...
    return new ImmutableFloat64HashSet(items, occupied, size, c);
  }
  ...
  select(predicate: (value: number) => boolean): Float64HashSet {
  ...
  reject(predicate: (value: number) => boolean): Float64HashSet {
    const result = new Float64HashSet();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.items[i])) {
        result.add(this.items[i]);
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  toMutable(): Float64HashSet {
    return Float64HashSet.fromImmutable(this);
  }
