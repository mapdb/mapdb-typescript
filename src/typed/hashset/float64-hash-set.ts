...
/**
 * Hash set backed by Float64Array with Uint8Array occupied bitmap.
 * Elements: 8 bytes each, contiguous memory, no GC pressure.
 * Open-addressing with linear probing.
 * Memory: 9 bytes/slot (vs ~50-70 bytes in Set<number>).
 */
export class Float64HashSet {
  ...
  constructor(capacity = DEFAULT_CAPACITY) {
    ...
    this.items = new Float64Array(this.capacity);
  ...
  static bulkLoadExact(
    ...
    const set = new Float64HashSet(hashCapacityFor(n));
   ...
   */
  static bulkLoad(values: Iterable<number>, opts?: BulkLoadOptions): Float64HashSet {
    const onDuplicate = opts?.onDuplicate ?? "error";
    ...
    const set =
      hint !== undefined ? new Float64HashSet(hashCapacityFor(hint)) : new Float64HashSet();
    let i = 0;
    for (const value of values) {
  ...
  union(other: Float64HashSet): Float64HashSet {
    const result = new Float64HashSet(this._size + other._size);
  ...
  *values(): Generator<number> {
  ...
  static fromImmutable(imm: {
    ...
    const set = new Float64HashSet(arr.length);
    for (const v of arr) {
      set.add(v);
    }
    return set;
