...
/**
 * Immutable hash set backed by Int32Array with Uint8Array occupied bitmap.
 ...
 */
export class ImmutableInt32HashSet {
  ...
  static of(values: number[]): ImmutableInt32HashSet {
    const set = new Int32HashSet(values.length);
    for (const v of values) {
      set.add(v);
    }
    return ImmutableInt32HashSet.fromMutable(set);
  }

  /** Creates an immutable copy from a mutable set (defensive copy). */
  static fromMutable(mutable: Int32HashSet): ImmutableInt32HashSet {
    const arr = mutable.toArray();
    const c = nextPowerOfTwo(Math.max(arr.length * 2, 16));
    const items = new Int32Array(c);
    ...
    return new ImmutableInt32HashSet(items, occupied, size, c);
  }
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  toMutable(): Int32HashSet {
    return Int32HashSet.fromImmutable(this);
  }
