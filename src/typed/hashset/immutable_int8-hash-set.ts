...
/**
 * Immutable hash set backed by Int8Array with Uint8Array occupied bitmap.
 ...
 */
export class ImmutableInt8HashSet {
  private items: Int8Array;
  private occupied: Uint8Array;
  private _size: number;
  private capacity: number;

  /** Creates an immutable set from an array of values (defensive copy). */
  static of(values: number[]): ImmutableInt8HashSet {
    const set = new Int8HashSet(values.length);
    for (const v of values) {
      set.add(v);
    }
    return ImmutableInt8HashSet.fromMutable(set);
  }

  /** Creates an immutable copy from a mutable set (defensive copy). */
  static fromMutable(mutable: Int8HashSet): ImmutableInt8HashSet {
    const arr = mutable.toArray();
    const c = nextPowerOfTwo(Math.max(arr.length * 2, 16));
    const items = new Int8Array(c);
    ...
    return new ImmutableInt8HashSet(items, occupied, size, c);
  }
  ...
  select(predicate: (value: number) => boolean): Int8HashSet {
  ...
  reject(predicate: (value: number) => boolean): Int8HashSet {
    const result = new Int8HashSet();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.items[i])) {
        result.add(this.items[i]);
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  toMutable(): Int8HashSet {
    return Int8HashSet.fromImmutable(this);
  }
