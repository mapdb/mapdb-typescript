...
import { ImmutableFloat32HashSet } from "./immutable_float32-hash-set.js";
import { Float32HashSet } from "./float32-hash-set.js";

describe("ImmutableFloat32HashSet generated", () => {
  it("static of creates immutable set", () => {
    const set = ImmutableFloat32HashSet.of([1, 2, 3]);
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new Float32HashSet();
    mutable.add(1);
    mutable.add(2);
    const imm = ImmutableFloat32HashSet.fromMutable(mutable);
    mutable.add(3);
  ...
  it("contains and includes", () => {
    const set = ImmutableFloat32HashSet.of([1, 2]);
    expect(set.contains(1)).toBe(true);
  ...
  it("size and isEmpty", () => {
    const empty = ImmutableFloat32HashSet.of([]);
    expect(empty.size()).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableFloat32HashSet.of([1]);
    expect(nonEmpty.size()).toBe(1);
    expect(nonEmpty.isEmpty()).toBe(false);
  });

  it("select returns MUTABLE", () => {
    const set = ImmutableFloat32HashSet.of([1, 2, 3, 4, 5]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableFloat32HashSet.of([1, 2, 3]);
  ...
  it("filter", () => {
    const set = ImmutableFloat32HashSet.of([1, 2, 3, 4]);
  ...
  it("defensive copy - modifying source array doesn't affect immutable", () => {
    const source = [1, 2, 3];
    const set = ImmutableFloat32HashSet.of(source);
    source[0] = 99 as any;
    expect(set.contains(1)).toBe(true);
    expect(set.contains(99)).toBe(false);
  });
  ...
  it("has no add method", () => {
    const set = ImmutableFloat32HashSet.of([1]);
