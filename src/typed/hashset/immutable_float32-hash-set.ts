...
/**
 * Immutable hash set backed by Float32Array with Uint8Array occupied bitmap.
 * Elements: 4 bytes each, contiguous memory.
 * Construct via static of(values) or fromMutable(mutable).
 * Mutations create new instances; select/reject return MUTABLE.
 */
export class ImmutableFloat32HashSet {
  ...
  static of(values: number[]): ImmutableFloat32HashSet {
    const set = new Float32HashSet(values.length);
    for (const v of values) {
      set.add(v);
    }
    return ImmutableFloat32HashSet.fromMutable(set);
  }

  /** Creates an immutable copy from a mutable set (defensive copy). */
  static fromMutable(mutable: Float32HashSet): ImmutableFloat32HashSet {
    const arr = mutable.toArray();
    const c = nextPowerOfTwo(Math.max(arr.length * 2, 16));
    const items = new Float32Array(c);
    ...
    return new ImmutableFloat32HashSet(items, occupied, size, c);
  }
  ...
  select(predicate: (value: number) => boolean): Float32HashSet {
    const result = new Float32HashSet();
  ...
  reject(predicate: (value: number) => boolean): Float32HashSet {
    const result = new Float32HashSet();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.items[i])) {
        result.add(this.items[i]);
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  toMutable(): Float32HashSet {
    return Float32HashSet.fromImmutable(this);
  }
