...
/**
 * Resizable list backed by Int8Array.
...
export class Int8ArrayList {
   ...
   */
  static bulkLoad(values: Iterable<number>): Int8ArrayList {
    const buffer = Array.from(values);
    const list = new Int8ArrayList(Math.max(buffer.length, 1));
  ...
  *entries(): Generator<[number, number]> {
  ...
  static fromImmutable(imm: { toArray(): number[] }): Int8ArrayList {
    const arr = imm.toArray();
    const list = new Int8ArrayList(arr.length);
    for (const v of arr) {
      list.add(v);
    }
    return list;
