...
/**
 * Resizable list backed by Float64Array.
...
export class Float64ArrayList {
   ...
   */
  static bulkLoad(values: Iterable<number>): Float64ArrayList {
    const buffer = Array.from(values);
    const list = new Float64ArrayList(Math.max(buffer.length, 1));
  ...
  *entries(): Generator<[number, number]> {
  ...
  static fromImmutable(imm: { toArray(): number[] }): Float64ArrayList {
    const arr = imm.toArray();
    const list = new Float64ArrayList(arr.length);
    for (const v of arr) {
      list.add(v);
    }
    return list;
