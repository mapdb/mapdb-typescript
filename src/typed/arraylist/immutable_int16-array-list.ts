...
import { Int16ArrayList } from "./int16-array-list.js";

/**
 * Immutable list backed by Int16Array.
 ...
 */
export class ImmutableInt16ArrayList {
  private data: Int16Array;
  private _size: number;

  /** Creates an immutable list from an array of values (defensive copy). */
  static of(values: number[]): ImmutableInt16ArrayList {
    const size = values.length;
    const data = new Int16Array(size);
    data.set(values);
    return new ImmutableInt16ArrayList(data, size);
  }

  /** Creates an immutable copy from a mutable list (defensive copy). */
  static fromMutable(mutable: Int16ArrayList): ImmutableInt16ArrayList {
    const arr = mutable.toArray();
    const data = new Int16Array(arr.length);
    data.set(arr);
    return new ImmutableInt16ArrayList(data, arr.length);
  }

  private constructor(data: Int16Array, size: number) {
    this.data = data;
    this._size = size;
  }
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  toMutable(): Int16ArrayList {
    return Int16ArrayList.fromImmutable(this);
  }
