...
import { ImmutableInt8ArrayList } from "./immutable_int8-array-list.js";
import { Int8ArrayList } from "./int8-array-list.js";

describe("ImmutableInt8ArrayList generated", () => {
  it("static of creates immutable copy", () => {
    const list = ImmutableInt8ArrayList.of([1, 2, 3]);
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new Int8ArrayList();
    mutable.add(1);
    mutable.add(2);
    const imm = ImmutableInt8ArrayList.fromMutable(mutable);
  ...
  it("get with bounds check", () => {
    const list = ImmutableInt8ArrayList.of([1, 2]);
    expect(list.get(0)).toBe(1);
  ...
  it("size and isEmpty", () => {
    const empty = ImmutableInt8ArrayList.of([]);
    expect(empty.size()).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableInt8ArrayList.of([1]);
  ...
  it("select returns MUTABLE", () => {
    const list = ImmutableInt8ArrayList.of([1, 2, 3, 4, 5]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableInt8ArrayList.of([1, 2, 3]);
  ...
  it("filter", () => {
    const list = ImmutableInt8ArrayList.of([1, 2, 3, 4]);
  ...
  it("memoryBytes", () => {
    const list = ImmutableInt8ArrayList.of([1, 2, 3]);
    // 3 elements * byteSize
    expect(list.memoryBytes()).toBe(3 * 1);
  });

  it("defensive copy - modifying source array doesn't affect immutable", () => {
    const source = [1, 2, 3];
    const list = ImmutableInt8ArrayList.of(source);
    source[0] = 99 as any;
    expect(list.get(0)).toBe(1);
  });
  ...
  it("has no add method", () => {
    const list = ImmutableInt8ArrayList.of([1]);
