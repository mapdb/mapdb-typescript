...
/**
 * Immutable list backed by Float32Array.
 ...
 */
export class ImmutableFloat32ArrayList {
  private data: Float32Array;
  private _size: number;

  /** Creates an immutable list from an array of values (defensive copy). */
  static of(values: number[]): ImmutableFloat32ArrayList {
    const size = values.length;
    const data = new Float32Array(size);
    data.set(values);
    return new ImmutableFloat32ArrayList(data, size);
  }

  /** Creates an immutable copy from a mutable list (defensive copy). */
  static fromMutable(mutable: Float32ArrayList): ImmutableFloat32ArrayList {
    const arr = mutable.toArray();
    const data = new Float32Array(arr.length);
    data.set(arr);
    return new ImmutableFloat32ArrayList(data, arr.length);
  }
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  toMutable(): Float32ArrayList {
    return Float32ArrayList.fromImmutable(this);
  }
