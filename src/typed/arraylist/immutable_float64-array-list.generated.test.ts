...
import { ImmutableFloat64ArrayList } from "./immutable_float64-array-list.js";
import { Float64ArrayList } from "./float64-array-list.js";

describe("ImmutableFloat64ArrayList generated", () => {
  it("static of creates immutable copy", () => {
    const list = ImmutableFloat64ArrayList.of([1, 2, 3]);
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new Float64ArrayList();
    mutable.add(1);
    mutable.add(2);
    const imm = ImmutableFloat64ArrayList.fromMutable(mutable);
  ...
  it("get with bounds check", () => {
    const list = ImmutableFloat64ArrayList.of([1, 2]);
    expect(list.get(0)).toBe(1);
  ...
  it("size and isEmpty", () => {
    const empty = ImmutableFloat64ArrayList.of([]);
    expect(empty.size()).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableFloat64ArrayList.of([1]);
  ...
  it("select returns MUTABLE", () => {
    const list = ImmutableFloat64ArrayList.of([1, 2, 3, 4, 5]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableFloat64ArrayList.of([1, 2, 3]);
  ...
  it("filter", () => {
    const list = ImmutableFloat64ArrayList.of([1, 2, 3, 4]);
  ...
  it("defensive copy - modifying source array doesn't affect immutable", () => {
    const source = [1, 2, 3];
    const list = ImmutableFloat64ArrayList.of(source);
    source[0] = 99 as any;
    expect(list.get(0)).toBe(1);
  });
  ...
  it("has no add method", () => {
    const list = ImmutableFloat64ArrayList.of([1]);
