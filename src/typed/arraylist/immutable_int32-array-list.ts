...
/**
 * Immutable list backed by Int32Array.
 ...
 */
export class ImmutableInt32ArrayList {
  private data: Int32Array;
  private _size: number;

  /** Creates an immutable list from an array of values (defensive copy). */
  static of(values: number[]): ImmutableInt32ArrayList {
    const size = values.length;
    const data = new Int32Array(size);
    data.set(values);
    return new ImmutableInt32ArrayList(data, size);
  }

  /** Creates an immutable copy from a mutable list (defensive copy). */
  static fromMutable(mutable: Int32ArrayList): ImmutableInt32ArrayList {
    const arr = mutable.toArray();
    const data = new Int32Array(arr.length);
    data.set(arr);
    return new ImmutableInt32ArrayList(data, arr.length);
  }
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  toMutable(): Int32ArrayList {
    return Int32ArrayList.fromImmutable(this);
  }
