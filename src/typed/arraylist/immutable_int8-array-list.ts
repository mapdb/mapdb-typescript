...
/**
 * Immutable list backed by Int8Array.
...
export class ImmutableInt8ArrayList {
  ...
  select(predicate: (value: number) => boolean): Int8ArrayList {
    const result = new Int8ArrayList();
    for (let i = 0; i < this._size; i++) {
  ...
  reject(predicate: (value: number) => boolean): Int8ArrayList {
    const result = new Int8ArrayList();
    for (let i = 0; i < this._size; i++) {
      if (!predicate(this.data[i])) result.add(this.data[i]);
    }
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  toMutable(): Int8ArrayList {
    return Int8ArrayList.fromImmutable(this);
  }
