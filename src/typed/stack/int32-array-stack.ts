...
export class Int32ArrayStack {
   ...
   */
  static bulkLoad(values: Iterable<number>): Int32ArrayStack {
    const buffer = Array.from(values);
    const stack = new Int32ArrayStack(Math.max(buffer.length, 1));
