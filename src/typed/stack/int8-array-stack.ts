...
/**
 * Array-backed stack (LIFO) backed by Int8Array.
 * Elements: 1 bytes each, contiguous memory, no GC pressure.
 * Grows by 2x when capacity is exceeded (allocates new Int8Array and copies).
 */
export class Int8ArrayStack {
  private data: Int8Array;
  private _size = 0;

  constructor(initialCapacity = DEFAULT_CAPACITY) {
    this.data = new Int8Array(Math.max(initialCapacity, 1));
  }

  /** Creates a new stack from an array of values. The last element becomes the top. */
  static of(values: number[]): Int8ArrayStack {
    const stack = new Int8ArrayStack(values.length);
   ...
   */
  static bulkLoad(values: Iterable<number>): Int8ArrayStack {
    const buffer = Array.from(values);
    const stack = new Int8ArrayStack(Math.max(buffer.length, 1));
  ...
  static fromMutable(imm: { toArray(): number[] }): Int8ArrayStack {
    const arr = imm.toArray();
    const stack = new Int8ArrayStack(arr.length);
    for (const v of arr) {
      stack.push(v);
    }
    return stack;
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  /** Creates a mutable stack from an immutable one. */
  static fromImmutable(imm: { toArray(): number[] }): Int8ArrayStack {
    const arr = imm.toArray();
    return Int8ArrayStack.of(arr);
  }
  ...
  private ensureCapacity(minCapacity: number): void {
    ...
    const newData = new Int8Array(newCapacity);
