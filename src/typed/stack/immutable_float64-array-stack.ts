...
/**
 * Immutable stack (LIFO) backed by Float64Array.
 ...
 */
export class ImmutableFloat64ArrayStack {
  private data: Float64Array;
  private _size: number;

  /** Creates an immutable stack from an array of values (defensive copy). Last element = top. */
  static of(values: number[]): ImmutableFloat64ArrayStack {
    const size = values.length;
    const data = new Float64Array(size);
    data.set(values);
    return new ImmutableFloat64ArrayStack(data, size);
  }

  /** Creates an immutable copy from a mutable stack (defensive copy). */
  static fromMutable(mutable: Float64ArrayStack): ImmutableFloat64ArrayStack {
    const arr = mutable.toArray();
    const data = new Float64Array(arr.length);
    data.set(arr);
    return new ImmutableFloat64ArrayStack(data, arr.length);
  }
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  toMutable(): Float64ArrayStack {
    return Float64ArrayStack.fromImmutable(this);
  }
