...
/**
 * Immutable stack (LIFO) backed by Int32Array.
 ...
 */
export class ImmutableInt32ArrayStack {
  private data: Int32Array;
  private _size: number;

  /** Creates an immutable stack from an array of values (defensive copy). Last element = top. */
  static of(values: number[]): ImmutableInt32ArrayStack {
    const size = values.length;
    const data = new Int32Array(size);
    data.set(values);
    return new ImmutableInt32ArrayStack(data, size);
  }

  /** Creates an immutable copy from a mutable stack (defensive copy). */
  static fromMutable(mutable: Int32ArrayStack): ImmutableInt32ArrayStack {
    const arr = mutable.toArray();
    const data = new Int32Array(arr.length);
    data.set(arr);
    return new ImmutableInt32ArrayStack(data, arr.length);
  }

  private constructor(data: Int32Array, size: number) {
    this.data = data;
    this._size = size;
  }
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  toMutable(): Int32ArrayStack {
    return Int32ArrayStack.fromImmutable(this);
  }
