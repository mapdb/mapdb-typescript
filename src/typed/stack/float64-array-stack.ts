...
/**
 * Array-backed stack (LIFO) backed by Float64Array.
 * Elements: 8 bytes each, contiguous memory, no GC pressure.
 * Grows by 2x when capacity is exceeded (allocates new Float64Array and copies).
 */
export class Float64ArrayStack {
  private data: Float64Array;
  private _size = 0;

  constructor(initialCapacity = DEFAULT_CAPACITY) {
    this.data = new Float64Array(Math.max(initialCapacity, 1));
  }

  /** Creates a new stack from an array of values. The last element becomes the top. */
  static of(values: number[]): Float64ArrayStack {
    const stack = new Float64ArrayStack(values.length);
   ...
   */
  static bulkLoad(values: Iterable<number>): Float64ArrayStack {
    const buffer = Array.from(values);
    const stack = new Float64ArrayStack(Math.max(buffer.length, 1));
  ...
  static fromMutable(imm: { toArray(): number[] }): Float64ArrayStack {
    const arr = imm.toArray();
    const stack = new Float64ArrayStack(arr.length);
    for (const v of arr) {
      stack.push(v);
    }
    return stack;
  ...
  select(predicate: (value: number) => boolean): Float64ArrayStack {
    const result = new Float64ArrayStack();
  ...
  reject(predicate: (value: number) => boolean): Float64ArrayStack {
    const result = new Float64ArrayStack();
    for (let i = 0; i < this._size; i++) {
      if (!predicate(this.data[i])) {
        result.push(this.data[i]);
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  /** Creates a mutable stack from an immutable one. */
  static fromImmutable(imm: { toArray(): number[] }): Float64ArrayStack {
    const arr = imm.toArray();
    return Float64ArrayStack.of(arr);
  }
