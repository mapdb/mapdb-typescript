...
/**
 * Array-backed stack (LIFO) backed by Float32Array.
 ...
 */
export class Float32ArrayStack {
  ...
  static of(values: number[]): Float32ArrayStack {
    const stack = new Float32ArrayStack(values.length);
  ...
  static fromMutable(imm: { toArray(): number[] }): Float32ArrayStack {
    ...
    const stack = new Float32ArrayStack(arr.length);
    for (const v of arr) {
      stack.push(v);
    }
    return stack;
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  /** Creates a mutable stack from an immutable one. */
  static fromImmutable(imm: { toArray(): number[] }): Float32ArrayStack {
    const arr = imm.toArray();
    return Float32ArrayStack.of(arr);
  }
