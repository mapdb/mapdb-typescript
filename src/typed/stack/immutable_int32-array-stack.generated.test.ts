...
import { ImmutableInt32ArrayStack } from "./immutable_int32-array-stack.js";
import { Int32ArrayStack } from "./int32-array-stack.js";

describe("ImmutableInt32ArrayStack generated", () => {
  it("static of creates immutable stack", () => {
    const stack = ImmutableInt32ArrayStack.of([1, 2, 3]); // last = top
    expect(stack.size()).toBe(3);
    expect(stack.peek()).toBe(3);
  });

  it("fromMutable creates defensive copy", () => {
    const mutable = new Int32ArrayStack();
    mutable.push(1);
    mutable.push(2);
    const imm = ImmutableInt32ArrayStack.fromMutable(mutable);
    mutable.push(3);
  ...
  it("peek returns top element", () => {
    const stack = ImmutableInt32ArrayStack.of([1, 2, 3]); // top = 3
    expect(stack.peek()).toBe(3);
  });

  it("peek throws on empty", () => {
    const stack = ImmutableInt32ArrayStack.of([]);
  ...
  it("peekAt throws on out of bounds", () => {
    const stack = ImmutableInt32ArrayStack.of([1]);
    expect(() => stack.peekAt(1)).toThrow();
    expect(() => stack.peekAt(-1)).toThrow();
  });

  it("size and isEmpty", () => {
    const empty = ImmutableInt32ArrayStack.of([]);
    expect(empty.size()).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableInt32ArrayStack.of([1]);
    expect(nonEmpty.size()).toBe(1);
    expect(nonEmpty.isEmpty()).toBe(false);
  });

  it("contains and includes", () => {
    const stack = ImmutableInt32ArrayStack.of([1, 2]);
  ...
  it("select returns MUTABLE", () => {
    const stack = ImmutableInt32ArrayStack.of([1, 2, 3, 4, 5]);
  ...
  it("every / some", () => {
    const stack = ImmutableInt32ArrayStack.of([1, 2, 3]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableInt32ArrayStack.of([1, 2, 3]);
  ...
  it("filter top to bottom", () => {
    const stack = ImmutableInt32ArrayStack.of([1, 2, 3, 4]);
  ...
  it("toString", () => {
    const stack = ImmutableInt32ArrayStack.of([1, 2]); // top = 2
  ...
  it("memoryBytes", () => {
    const stack = ImmutableInt32ArrayStack.of([1, 2, 3]);
    expect(stack.memoryBytes()).toBe(3 * 4);
  });

  it("defensive copy - modifying source array doesn't affect immutable", () => {
    const source = [1, 2, 3];
    const stack = ImmutableInt32ArrayStack.of(source);
    source[0] = 99 as any;
    expect(stack.peek()).toBe(3);
  });
