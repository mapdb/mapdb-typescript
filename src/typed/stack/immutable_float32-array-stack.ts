...
/**
 * Immutable stack (LIFO) backed by Float32Array.
 ...
 */
export class ImmutableFloat32ArrayStack {
  private data: Float32Array;
  private _size: number;

  /** Creates an immutable stack from an array of values (defensive copy). Last element = top. */
  static of(values: number[]): ImmutableFloat32ArrayStack {
    const size = values.length;
    const data = new Float32Array(size);
    data.set(values);
    return new ImmutableFloat32ArrayStack(data, size);
  }

  /** Creates an immutable copy from a mutable stack (defensive copy). */
  static fromMutable(mutable: Float32ArrayStack): ImmutableFloat32ArrayStack {
    const arr = mutable.toArray();
    const data = new Float32Array(arr.length);
    data.set(arr);
    return new ImmutableFloat32ArrayStack(data, arr.length);
  }

  private constructor(data: Float32Array, size: number) {
    this.data = data;
    this._size = size;
  }
  ...
  select(predicate: (value: number) => boolean): Float32ArrayStack {
    const result = new Float32ArrayStack();
    for (let i = 0; i < this._size; i++) {
  ...
  reject(predicate: (value: number) => boolean): Float32ArrayStack {
    const result = new Float32ArrayStack();
    for (let i = 0; i < this._size; i++) {
      if (!predicate(this.data[i])) {
        result.push(this.data[i]);
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  toMutable(): Float32ArrayStack {
    return Float32ArrayStack.fromImmutable(this);
  }
