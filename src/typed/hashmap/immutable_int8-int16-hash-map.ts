...
import { Int8Int16HashMap } from "./int8-int16-hash-map.js";

/**
 * Immutable Int8→Int16 hash map backed by Int8Array and Int16Array.
 * Keys: 1 bytes each, values: 2 bytes each.
 * Construct via static of(entries) or fromMutable(mutable).
 * Mutations create new instances; select/reject return MUTABLE.
 * Memory: 4 bytes/slot (vs ~50-70 bytes in Map<number, number>).
 */
export class ImmutableInt8Int16HashMap {
  private keys: Int8Array;
  ...
  static of(entries: [number, number][]): ImmutableInt8Int16HashMap {
    const map = new Int8Int16HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt8Int16HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int8Int16HashMap): ImmutableInt8Int16HashMap {
    const entries: [number, number][] = [...mutable.entries()];
    const c = nextPowerOfTwo(Math.max(entries.length * 2, 16));
    const keys = new Int8Array(c);
    ...
    return new ImmutableInt8Int16HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int8Int16HashMap {
    const result = new Int8Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int8Int16HashMap {
    const result = new Int8Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
