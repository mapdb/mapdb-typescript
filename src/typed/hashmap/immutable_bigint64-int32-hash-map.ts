...
/**
 * Immutable BigInt64→Int32 hash map backed by BigInt64Array and Int32Array.
 ...
 */
export class ImmutableBigInt64Int32HashMap {
  ...
  static of(entries: [bigint, number][]): ImmutableBigInt64Int32HashMap {
    const map = new BigInt64Int32HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableBigInt64Int32HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: BigInt64Int32HashMap): ImmutableBigInt64Int32HashMap {
    ...
    return new ImmutableBigInt64Int32HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: bigint, value: number) => boolean): BigInt64Int32HashMap {
    const result = new BigInt64Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: bigint, value: number) => boolean): BigInt64Int32HashMap {
    const result = new BigInt64Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
  ...
  toMutable(): BigInt64Int32HashMap {
    const result = new BigInt64Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
