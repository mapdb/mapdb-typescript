...
import { Float64Float64HashMap } from "./float64-float64-hash-map.js";

/**
 * Immutable Float64→Float64 hash map backed by Float64Array and Float64Array.
 ...
 * Memory: 17 bytes/slot (vs ~50-70 bytes in Map<number, number>).
 */
export class ImmutableFloat64Float64HashMap {
  ...
  static of(entries: [number, number][]): ImmutableFloat64Float64HashMap {
    const map = new Float64Float64HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableFloat64Float64HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Float64Float64HashMap): ImmutableFloat64Float64HashMap {
    ...
    return new ImmutableFloat64Float64HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Float64Float64HashMap {
    const result = new Float64Float64HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Float64Float64HashMap {
    const result = new Float64Float64HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
  ...
  toMutable(): Float64Float64HashMap {
    const result = new Float64Float64HashMap();
    for (let i = 0; i < this.capacity; i++) {
