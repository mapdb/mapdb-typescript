...
import { Float64Int16HashMap } from "./float64-int16-hash-map.js";

/**
 * Immutable Float64→Int16 hash map backed by Float64Array and Int16Array.
 ...
 * Memory: 11 bytes/slot (vs ~50-70 bytes in Map<number, number>).
 */
export class ImmutableFloat64Int16HashMap {
  ...
  static of(entries: [number, number][]): ImmutableFloat64Int16HashMap {
    const map = new Float64Int16HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableFloat64Int16HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Float64Int16HashMap): ImmutableFloat64Int16HashMap {
    ...
    return new ImmutableFloat64Int16HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Float64Int16HashMap {
    const result = new Float64Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Float64Int16HashMap {
    const result = new Float64Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
