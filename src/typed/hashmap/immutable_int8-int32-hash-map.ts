...
import { Int8Int32HashMap } from "./int8-int32-hash-map.js";

/**
 * Immutable Int8→Int32 hash map backed by Int8Array and Int32Array.
 ...
 */
export class ImmutableInt8Int32HashMap {
  ...
  static of(entries: [number, number][]): ImmutableInt8Int32HashMap {
    const map = new Int8Int32HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt8Int32HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int8Int32HashMap): ImmutableInt8Int32HashMap {
    ...
    return new ImmutableInt8Int32HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int8Int32HashMap {
    const result = new Int8Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int8Int32HashMap {
    const result = new Int8Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
