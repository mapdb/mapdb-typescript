...
import { Float32Float32HashMap } from "./float32-float32-hash-map.js";

/**
 * Immutable Float32→Float32 hash map backed by Float32Array and Float32Array.
 * Keys: 4 bytes each, values: 4 bytes each.
 * Construct via static of(entries) or fromMutable(mutable).
 * Mutations create new instances; select/reject return MUTABLE.
 * Memory: 9 bytes/slot (vs ~50-70 bytes in Map<number, number>).
 */
export class ImmutableFloat32Float32HashMap {
  ...
  static of(entries: [number, number][]): ImmutableFloat32Float32HashMap {
    const map = new Float32Float32HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableFloat32Float32HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Float32Float32HashMap): ImmutableFloat32Float32HashMap {
    ...
    return new ImmutableFloat32Float32HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Float32Float32HashMap {
    const result = new Float32Float32HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Float32Float32HashMap {
    const result = new Float32Float32HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
  ...
  toMutable(): Float32Float32HashMap {
    const result = new Float32Float32HashMap();
    for (let i = 0; i < this.capacity; i++) {
