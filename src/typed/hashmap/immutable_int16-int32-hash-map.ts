...
import { Int16Int32HashMap } from "./int16-int32-hash-map.js";

/**
 * Immutable Int16→Int32 hash map backed by Int16Array and Int32Array.
 * Keys: 2 bytes each, values: 4 bytes each.
 ...
 */
export class ImmutableInt16Int32HashMap {
  ...
  static of(entries: [number, number][]): ImmutableInt16Int32HashMap {
    const map = new Int16Int32HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt16Int32HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int16Int32HashMap): ImmutableInt16Int32HashMap {
    ...
    return new ImmutableInt16Int32HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int16Int32HashMap {
    const result = new Int16Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int16Int32HashMap {
    const result = new Int16Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
