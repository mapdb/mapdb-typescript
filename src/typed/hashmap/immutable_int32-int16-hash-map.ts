...
import { Int32Int16HashMap } from "./int32-int16-hash-map.js";

/**
 * Immutable Int32→Int16 hash map backed by Int32Array and Int16Array.
 ...
 */
export class ImmutableInt32Int16HashMap {
  ...
  static of(entries: [number, number][]): ImmutableInt32Int16HashMap {
    const map = new Int32Int16HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt32Int16HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int32Int16HashMap): ImmutableInt32Int16HashMap {
    ...
    return new ImmutableInt32Int16HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int32Int16HashMap {
    const result = new Int32Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int32Int16HashMap {
    const result = new Int32Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
