...
/**
 * Float32→Float32 hash map backed by Float32Array and Float32Array.
 ...
 */
export class Float32Float32HashMap {
  ...
  ): Float32Float32HashMap {
    checkExpectedSize(n);
    const onDuplicate = opts?.onDuplicate ?? "error";
    const map = new Float32Float32HashMap(hashCapacityFor(n));
  ...
  static bulkLoad(
    ...
    return Float32Float32HashMap.bulkLoadExact(buffer, buffer.length, opts);
  }
