...
/**
 * Immutable Int32→BigInt64 hash map backed by Int32Array and BigInt64Array.
 ...
 */
export class ImmutableInt32BigInt64HashMap {
  ...
  static of(entries: [number, bigint][]): ImmutableInt32BigInt64HashMap {
    const map = new Int32BigInt64HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt32BigInt64HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int32BigInt64HashMap): ImmutableInt32BigInt64HashMap {
    ...
    return new ImmutableInt32BigInt64HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<bigint> {
  ...
  select(predicate: (key: number, value: bigint) => boolean): Int32BigInt64HashMap {
    const result = new Int32BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: bigint) => boolean): Int32BigInt64HashMap {
    const result = new Int32BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
  ...
  toMutable(): Int32BigInt64HashMap {
    const result = new Int32BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
