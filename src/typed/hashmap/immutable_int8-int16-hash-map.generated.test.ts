...
import { ImmutableInt8Int16HashMap } from "./immutable_int8-int16-hash-map.js";
import { Int8Int16HashMap } from "./int8-int16-hash-map.js";

describe("ImmutableInt8Int16HashMap generated", () => {
  it("static of creates immutable map", () => {
    const m = ImmutableInt8Int16HashMap.of([[1, 1], [2, 2], [3, 3]]);
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new Int8Int16HashMap();
    mutable.set(1, 1);
    mutable.set(2, 2);
    const imm = ImmutableInt8Int16HashMap.fromMutable(mutable);
    mutable.set(3, 3);
  ...
  it("get and has", () => {
    const m = ImmutableInt8Int16HashMap.of([[1, 1], [2, 2]]);
    expect(m.get(1)).toBe(1);
  ...
  it("getOrDefault", () => {
    const m = ImmutableInt8Int16HashMap.of([[1, 1]]);
    expect(m.getOrDefault(1, 3)).toBe(1);
    expect(m.getOrDefault(99, 3)).toBe(3);
  });

  it("size and isEmpty", () => {
    const empty = ImmutableInt8Int16HashMap.of([]);
    expect(empty.size).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableInt8Int16HashMap.of([[1, 1]]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableInt8Int16HashMap.of([[1, 1], [2, 2]]);
