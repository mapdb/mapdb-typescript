...
/**
 * Immutable Int32→Int8 hash map backed by Int32Array and Int8Array.
 ...
 */
export class ImmutableInt32Int8HashMap {
  ...
  static of(entries: [number, number][]): ImmutableInt32Int8HashMap {
    const map = new Int32Int8HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt32Int8HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int32Int8HashMap): ImmutableInt32Int8HashMap {
    ...
    return new ImmutableInt32Int8HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int32Int8HashMap {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int32Int8HashMap {
    const result = new Int32Int8HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
