...
/**
 * Immutable BigInt64→Int16 hash map backed by BigInt64Array and Int16Array.
 * Keys: 8 bytes each, values: 2 bytes each.
 * Construct via static of(entries) or fromMutable(mutable).
 * Mutations create new instances; select/reject return MUTABLE.
 * Memory: 11 bytes/slot (vs ~50-70 bytes in Map<bigint, number>).
 */
export class ImmutableBigInt64Int16HashMap {
  ...
  static of(entries: [bigint, number][]): ImmutableBigInt64Int16HashMap {
    const map = new BigInt64Int16HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableBigInt64Int16HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: BigInt64Int16HashMap): ImmutableBigInt64Int16HashMap {
    ...
    return new ImmutableBigInt64Int16HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: bigint, value: number) => boolean): BigInt64Int16HashMap {
    const result = new BigInt64Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: bigint, value: number) => boolean): BigInt64Int16HashMap {
    const result = new BigInt64Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
  ...
  toMutable(): BigInt64Int16HashMap {
    const result = new BigInt64Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
