...
import { BigInt64Float32HashMap } from "./bigint64-float32-hash-map.js";

/**
 * Immutable BigInt64→Float32 hash map backed by BigInt64Array and Float32Array.
 ...
 * Memory: 13 bytes/slot (vs ~50-70 bytes in Map<bigint, number>).
 */
export class ImmutableBigInt64Float32HashMap {
  ...
  static of(entries: [bigint, number][]): ImmutableBigInt64Float32HashMap {
    const map = new BigInt64Float32HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableBigInt64Float32HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: BigInt64Float32HashMap): ImmutableBigInt64Float32HashMap {
    ...
    const values = new Float32Array(c);
    ...
    return new ImmutableBigInt64Float32HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: bigint, value: number) => boolean): BigInt64Float32HashMap {
    const result = new BigInt64Float32HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: bigint, value: number) => boolean): BigInt64Float32HashMap {
    const result = new BigInt64Float32HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
  ...
  toMutable(): BigInt64Float32HashMap {
    const result = new BigInt64Float32HashMap();
    for (let i = 0; i < this.capacity; i++) {
