...
import { Float64BigInt64HashMap } from "./float64-bigint64-hash-map.js";

/**
 * Immutable Float64→BigInt64 hash map backed by Float64Array and BigInt64Array.
 ...
 * Memory: 17 bytes/slot (vs ~50-70 bytes in Map<number, bigint>).
 */
export class ImmutableFloat64BigInt64HashMap {
  ...
  static of(entries: [number, bigint][]): ImmutableFloat64BigInt64HashMap {
    const map = new Float64BigInt64HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableFloat64BigInt64HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Float64BigInt64HashMap): ImmutableFloat64BigInt64HashMap {
    ...
    return new ImmutableFloat64BigInt64HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<bigint> {
  ...
  select(predicate: (key: number, value: bigint) => boolean): Float64BigInt64HashMap {
  ...
  reject(predicate: (key: number, value: bigint) => boolean): Float64BigInt64HashMap {
    const result = new Float64BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
  ...
  toMutable(): Float64BigInt64HashMap {
    const result = new Float64BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
