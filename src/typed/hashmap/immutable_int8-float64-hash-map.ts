...
/**
 * Immutable Int8→Float64 hash map backed by Int8Array and Float64Array.
 ...
 * Memory: 10 bytes/slot (vs ~50-70 bytes in Map<number, number>).
 */
export class ImmutableInt8Float64HashMap {
  ...
  static of(entries: [number, number][]): ImmutableInt8Float64HashMap {
    const map = new Int8Float64HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt8Float64HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int8Float64HashMap): ImmutableInt8Float64HashMap {
    ...
    return new ImmutableInt8Float64HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int8Float64HashMap {
    const result = new Int8Float64HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int8Float64HashMap {
    const result = new Int8Float64HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
