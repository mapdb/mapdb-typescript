...
import { ImmutableFloat32Int8HashMap } from "./immutable_float32-int8-hash-map.js";
import { Float32Int8HashMap } from "./float32-int8-hash-map.js";

describe("ImmutableFloat32Int8HashMap generated", () => {
  it("static of creates immutable map", () => {
    const m = ImmutableFloat32Int8HashMap.of([[1, 1], [2, 2], [3, 3]]);
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new Float32Int8HashMap();
    mutable.set(1, 1);
    mutable.set(2, 2);
    const imm = ImmutableFloat32Int8HashMap.fromMutable(mutable);
    mutable.set(3, 3);
  ...
  it("get and has", () => {
    const m = ImmutableFloat32Int8HashMap.of([[1, 1], [2, 2]]);
    expect(m.get(1)).toBe(1);
  ...
  it("getOrDefault", () => {
    const m = ImmutableFloat32Int8HashMap.of([[1, 1]]);
    expect(m.getOrDefault(1, 3)).toBe(1);
    expect(m.getOrDefault(99, 3)).toBe(3);
  });

  it("size and isEmpty", () => {
    const empty = ImmutableFloat32Int8HashMap.of([]);
    expect(empty.size).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableFloat32Int8HashMap.of([[1, 1]]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableFloat32Int8HashMap.of([[1, 1], [2, 2]]);
