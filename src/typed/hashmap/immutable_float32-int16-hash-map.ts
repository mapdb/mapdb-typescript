...
import { Float32Int16HashMap } from "./float32-int16-hash-map.js";

/**
 * Immutable Float32→Int16 hash map backed by Float32Array and Int16Array.
 * Keys: 4 bytes each, values: 2 bytes each.
 * Construct via static of(entries) or fromMutable(mutable).
 * Mutations create new instances; select/reject return MUTABLE.
 * Memory: 7 bytes/slot (vs ~50-70 bytes in Map<number, number>).
 */
export class ImmutableFloat32Int16HashMap {
  ...
  static of(entries: [number, number][]): ImmutableFloat32Int16HashMap {
    const map = new Float32Int16HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableFloat32Int16HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Float32Int16HashMap): ImmutableFloat32Int16HashMap {
    ...
    return new ImmutableFloat32Int16HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Float32Int16HashMap {
    const result = new Float32Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Float32Int16HashMap {
    const result = new Float32Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
