...
/**
 * Immutable Int8→BigInt64 hash map backed by Int8Array and BigInt64Array.
 * Keys: 1 bytes each, values: 8 bytes each.
 * Construct via static of(entries) or fromMutable(mutable).
 * Mutations create new instances; select/reject return MUTABLE.
 * Memory: 10 bytes/slot (vs ~50-70 bytes in Map<number, bigint>).
 */
export class ImmutableInt8BigInt64HashMap {
  ...
  static of(entries: [number, bigint][]): ImmutableInt8BigInt64HashMap {
    const map = new Int8BigInt64HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt8BigInt64HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int8BigInt64HashMap): ImmutableInt8BigInt64HashMap {
    ...
    return new ImmutableInt8BigInt64HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<bigint> {
  ...
  select(predicate: (key: number, value: bigint) => boolean): Int8BigInt64HashMap {
    const result = new Int8BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: bigint) => boolean): Int8BigInt64HashMap {
    const result = new Int8BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
