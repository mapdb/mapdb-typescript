...
import { Int8Int8HashMap } from "./int8-int8-hash-map.js";

/**
 * Immutable Int8→Int8 hash map backed by Int8Array and Int8Array.
 * Keys: 1 bytes each, values: 1 bytes each.
 * Construct via static of(entries) or fromMutable(mutable).
 * Mutations create new instances; select/reject return MUTABLE.
 * Memory: 3 bytes/slot (vs ~50-70 bytes in Map<number, number>).
 */
export class ImmutableInt8Int8HashMap {
  ...
  static of(entries: [number, number][]): ImmutableInt8Int8HashMap {
    const map = new Int8Int8HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt8Int8HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int8Int8HashMap): ImmutableInt8Int8HashMap {
    ...
    return new ImmutableInt8Int8HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int8Int8HashMap {
    const result = new Int8Int8HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int8Int8HashMap {
    const result = new Int8Int8HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
