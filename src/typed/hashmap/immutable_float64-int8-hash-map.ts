...
import { Float64Int8HashMap } from "./float64-int8-hash-map.js";

/**
 * Immutable Float64→Int8 hash map backed by Float64Array and Int8Array.
 ...
 */
export class ImmutableFloat64Int8HashMap {
  ...
  static of(entries: [number, number][]): ImmutableFloat64Int8HashMap {
    const map = new Float64Int8HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableFloat64Int8HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Float64Int8HashMap): ImmutableFloat64Int8HashMap {
    ...
    return new ImmutableFloat64Int8HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Float64Int8HashMap {
    const result = new Float64Int8HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Float64Int8HashMap {
    const result = new Float64Int8HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
