...
/**
 * Int32→BigInt64 hash map backed by Int32Array and BigInt64Array.
 ...
 */
export class Int32BigInt64HashMap {
  ...
  ): Int32BigInt64HashMap {
    checkExpectedSize(n);
    const onDuplicate = opts?.onDuplicate ?? "error";
    const map = new Int32BigInt64HashMap(hashCapacityFor(n));
  ...
  static bulkLoad(
    ...
    const map =
      hint !== undefined ? new Int32BigInt64HashMap(hashCapacityFor(hint)) : new Int32BigInt64HashMap();
    let i = 0;
    for (const [key, value] of pairs) {
