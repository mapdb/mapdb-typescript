...
/**
 * BigInt64→Float64 hash map backed by BigInt64Array and Float64Array.
 ...
 */
export class BigInt64Float64HashMap {
  ...
  ): BigInt64Float64HashMap {
    checkExpectedSize(n);
    const onDuplicate = opts?.onDuplicate ?? "error";
    const map = new BigInt64Float64HashMap(hashCapacityFor(n));
  ...
  static bulkLoad(
    ...
    const map =
      hint !== undefined ? new BigInt64Float64HashMap(hashCapacityFor(hint)) : new BigInt64Float64HashMap();
    let i = 0;
    for (const [key, value] of pairs) {
