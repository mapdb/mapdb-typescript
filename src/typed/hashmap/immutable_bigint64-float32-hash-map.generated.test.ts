...
import { ImmutableBigInt64Float32HashMap } from "./immutable_bigint64-float32-hash-map.js";
import { BigInt64Float32HashMap } from "./bigint64-float32-hash-map.js";

describe("ImmutableBigInt64Float32HashMap generated", () => {
  it("static of creates immutable map", () => {
    const m = ImmutableBigInt64Float32HashMap.of([[1n, 1], [2n, 2], [3n, 3]]);
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new BigInt64Float32HashMap();
    mutable.set(1n, 1);
    mutable.set(2n, 2);
    const imm = ImmutableBigInt64Float32HashMap.fromMutable(mutable);
    mutable.set(3n, 3);
  ...
  it("get and has", () => {
    const m = ImmutableBigInt64Float32HashMap.of([[1n, 1], [2n, 2]]);
    expect(m.get(1n)).toBe(1);
  ...
  it("getOrDefault", () => {
    const m = ImmutableBigInt64Float32HashMap.of([[1n, 1]]);
    expect(m.getOrDefault(1n, 3)).toBe(1);
    expect(m.getOrDefault(99n, 3)).toBe(3);
  });

  it("size and isEmpty", () => {
    const empty = ImmutableBigInt64Float32HashMap.of([]);
    expect(empty.size).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableBigInt64Float32HashMap.of([[1n, 1]]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableBigInt64Float32HashMap.of([[1n, 1], [2n, 2]]);
