...
/**
 * Immutable Int8→Float32 hash map backed by Int8Array and Float32Array.
 * Keys: 1 bytes each, values: 4 bytes each.
 ...
 */
export class ImmutableInt8Float32HashMap {
  ...
  static of(entries: [number, number][]): ImmutableInt8Float32HashMap {
    const map = new Int8Float32HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt8Float32HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int8Float32HashMap): ImmutableInt8Float32HashMap {
    ...
    return new ImmutableInt8Float32HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int8Float32HashMap {
    const result = new Int8Float32HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int8Float32HashMap {
    const result = new Int8Float32HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
