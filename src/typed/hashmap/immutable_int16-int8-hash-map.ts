...
import { Int16Int8HashMap } from "./int16-int8-hash-map.js";

/**
 * Immutable Int16→Int8 hash map backed by Int16Array and Int8Array.
 * Keys: 2 bytes each, values: 1 bytes each.
 ...
 */
export class ImmutableInt16Int8HashMap {
  ...
  static of(entries: [number, number][]): ImmutableInt16Int8HashMap {
    const map = new Int16Int8HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt16Int8HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int16Int8HashMap): ImmutableInt16Int8HashMap {
    ...
    return new ImmutableInt16Int8HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int16Int8HashMap {
    const result = new Int16Int8HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int16Int8HashMap {
    const result = new Int16Int8HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
