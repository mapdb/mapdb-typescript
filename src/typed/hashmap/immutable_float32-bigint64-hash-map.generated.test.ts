...
import { ImmutableFloat32BigInt64HashMap } from "./immutable_float32-bigint64-hash-map.js";
import { Float32BigInt64HashMap } from "./float32-bigint64-hash-map.js";

describe("ImmutableFloat32BigInt64HashMap generated", () => {
  it("static of creates immutable map", () => {
    const m = ImmutableFloat32BigInt64HashMap.of([[1, 1n], [2, 2n], [3, 3n]]);
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new Float32BigInt64HashMap();
    mutable.set(1, 1n);
    mutable.set(2, 2n);
    const imm = ImmutableFloat32BigInt64HashMap.fromMutable(mutable);
    mutable.set(3, 3n);
  ...
  it("get and has", () => {
    const m = ImmutableFloat32BigInt64HashMap.of([[1, 1n], [2, 2n]]);
    expect(m.get(1)).toBe(1n);
  ...
  it("getOrDefault", () => {
    const m = ImmutableFloat32BigInt64HashMap.of([[1, 1n]]);
    expect(m.getOrDefault(1, 3n)).toBe(1n);
    expect(m.getOrDefault(99, 3n)).toBe(3n);
  });

  it("size and isEmpty", () => {
    const empty = ImmutableFloat32BigInt64HashMap.of([]);
    expect(empty.size).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableFloat32BigInt64HashMap.of([[1, 1n]]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableFloat32BigInt64HashMap.of([[1, 1n], [2, 2n]]);
