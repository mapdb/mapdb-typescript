...
import { Int16Float64HashMap } from "./int16-float64-hash-map.js";

/**
 * Immutable Int16→Float64 hash map backed by Int16Array and Float64Array.
 ...
 */
export class ImmutableInt16Float64HashMap {
  ...
  static of(entries: [number, number][]): ImmutableInt16Float64HashMap {
    const map = new Int16Float64HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt16Float64HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int16Float64HashMap): ImmutableInt16Float64HashMap {
    ...
    return new ImmutableInt16Float64HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int16Float64HashMap {
    const result = new Int16Float64HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int16Float64HashMap {
    const result = new Int16Float64HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
