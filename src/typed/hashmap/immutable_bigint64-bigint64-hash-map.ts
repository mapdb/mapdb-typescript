...
/**
 * Immutable BigInt64→BigInt64 hash map backed by BigInt64Array and BigInt64Array.
 * Keys: 8 bytes each, values: 8 bytes each.
 * Construct via static of(entries) or fromMutable(mutable).
 * Mutations create new instances; select/reject return MUTABLE.
 * Memory: 17 bytes/slot (vs ~50-70 bytes in Map<bigint, bigint>).
 */
export class ImmutableBigInt64BigInt64HashMap {
  ...
  static of(entries: [bigint, bigint][]): ImmutableBigInt64BigInt64HashMap {
    const map = new BigInt64BigInt64HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableBigInt64BigInt64HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: BigInt64BigInt64HashMap): ImmutableBigInt64BigInt64HashMap {
    const entries: [bigint, bigint][] = [...mutable.entries()];
    ...
    return new ImmutableBigInt64BigInt64HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<bigint> {
  ...
  select(predicate: (key: bigint, value: bigint) => boolean): BigInt64BigInt64HashMap {
    const result = new BigInt64BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: bigint, value: bigint) => boolean): BigInt64BigInt64HashMap {
    const result = new BigInt64BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
  ...
  toMutable(): BigInt64BigInt64HashMap {
    const result = new BigInt64BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
