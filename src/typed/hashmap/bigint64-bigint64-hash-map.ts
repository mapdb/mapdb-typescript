...
/**
 * BigInt64→BigInt64 hash map backed by BigInt64Array and BigInt64Array.
 ...
 */
export class BigInt64BigInt64HashMap {
  ...
  ): BigInt64BigInt64HashMap {
    checkExpectedSize(n);
    const onDuplicate = opts?.onDuplicate ?? "error";
    const map = new BigInt64BigInt64HashMap(hashCapacityFor(n));
  ...
  static bulkLoad(
    ...
    const map =
      hint !== undefined ? new BigInt64BigInt64HashMap(hashCapacityFor(hint)) : new BigInt64BigInt64HashMap();
    let i = 0;
    for (const [key, value] of pairs) {
