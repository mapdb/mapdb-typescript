...
/**
 * Immutable BigInt64→Float64 hash map backed by BigInt64Array and Float64Array.
 ...
 * Memory: 17 bytes/slot (vs ~50-70 bytes in Map<bigint, number>).
 */
export class ImmutableBigInt64Float64HashMap {
  ...
  static of(entries: [bigint, number][]): ImmutableBigInt64Float64HashMap {
    const map = new BigInt64Float64HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableBigInt64Float64HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: BigInt64Float64HashMap): ImmutableBigInt64Float64HashMap {
    ...
    return new ImmutableBigInt64Float64HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: bigint, value: number) => boolean): BigInt64Float64HashMap {
    const result = new BigInt64Float64HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: bigint, value: number) => boolean): BigInt64Float64HashMap {
    const result = new BigInt64Float64HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
  ...
  toMutable(): BigInt64Float64HashMap {
    const result = new BigInt64Float64HashMap();
    for (let i = 0; i < this.capacity; i++) {
