...
import { Float32Float64HashMap } from "./float32-float64-hash-map.js";

/**
 * Immutable Float32→Float64 hash map backed by Float32Array and Float64Array.
 ...
 */
export class ImmutableFloat32Float64HashMap {
  ...
  static of(entries: [number, number][]): ImmutableFloat32Float64HashMap {
    const map = new Float32Float64HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableFloat32Float64HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Float32Float64HashMap): ImmutableFloat32Float64HashMap {
    ...
    return new ImmutableFloat32Float64HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Float32Float64HashMap {
  ...
  reject(predicate: (key: number, value: number) => boolean): Float32Float64HashMap {
    const result = new Float32Float64HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
  ...
  toMutable(): Float32Float64HashMap {
    const result = new Float32Float64HashMap();
    for (let i = 0; i < this.capacity; i++) {
