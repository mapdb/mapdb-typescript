...
/**
 * Immutable Int16→BigInt64 hash map backed by Int16Array and BigInt64Array.
 * Keys: 2 bytes each, values: 8 bytes each.
 * Construct via static of(entries) or fromMutable(mutable).
 * Mutations create new instances; select/reject return MUTABLE.
 * Memory: 11 bytes/slot (vs ~50-70 bytes in Map<number, bigint>).
 */
export class ImmutableInt16BigInt64HashMap {
  ...
  static of(entries: [number, bigint][]): ImmutableInt16BigInt64HashMap {
    const map = new Int16BigInt64HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt16BigInt64HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int16BigInt64HashMap): ImmutableInt16BigInt64HashMap {
    const entries: [number, bigint][] = [...mutable.entries()];
    const c = nextPowerOfTwo(Math.max(entries.length * 2, 16));
    const keys = new Int16Array(c);
    ...
    return new ImmutableInt16BigInt64HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<bigint> {
  ...
  select(predicate: (key: number, value: bigint) => boolean): Int16BigInt64HashMap {
    const result = new Int16BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: bigint) => boolean): Int16BigInt64HashMap {
    const result = new Int16BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
  ...
  toMutable(): Int16BigInt64HashMap {
    const result = new Int16BigInt64HashMap();
    for (let i = 0; i < this.capacity; i++) {
