...
/**
 * Immutable Float32→Int32 hash map backed by Float32Array and Int32Array.
 ...
 */
export class ImmutableFloat32Int32HashMap {
  ...
  static of(entries: [number, number][]): ImmutableFloat32Int32HashMap {
    const map = new Float32Int32HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableFloat32Int32HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Float32Int32HashMap): ImmutableFloat32Int32HashMap {
    ...
    return new ImmutableFloat32Int32HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Float32Int32HashMap {
    const result = new Float32Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Float32Int32HashMap {
    const result = new Float32Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
