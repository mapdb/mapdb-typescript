...
import { Int16Int16HashMap } from "./int16-int16-hash-map.js";

/**
 * Immutable Int16→Int16 hash map backed by Int16Array and Int16Array.
 * Keys: 2 bytes each, values: 2 bytes each.
 * Construct via static of(entries) or fromMutable(mutable).
 * Mutations create new instances; select/reject return MUTABLE.
 * Memory: 5 bytes/slot (vs ~50-70 bytes in Map<number, number>).
 */
export class ImmutableInt16Int16HashMap {
  ...
  static of(entries: [number, number][]): ImmutableInt16Int16HashMap {
    const map = new Int16Int16HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt16Int16HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int16Int16HashMap): ImmutableInt16Int16HashMap {
    ...
    return new ImmutableInt16Int16HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int16Int16HashMap {
    const result = new Int16Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int16Int16HashMap {
    const result = new Int16Int16HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
