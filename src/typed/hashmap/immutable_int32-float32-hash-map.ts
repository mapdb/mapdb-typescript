...
/**
 * Immutable Int32→Float32 hash map backed by Int32Array and Float32Array.
 ...
 */
export class ImmutableInt32Float32HashMap {
  ...
  static of(entries: [number, number][]): ImmutableInt32Float32HashMap {
    const map = new Int32Float32HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt32Float32HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int32Float32HashMap): ImmutableInt32Float32HashMap {
    ...
    return new ImmutableInt32Float32HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int32Float32HashMap {
    const result = new Int32Float32HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int32Float32HashMap {
    const result = new Int32Float32HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
