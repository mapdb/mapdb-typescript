...
/**
 * BigInt64→Float32 hash map backed by BigInt64Array and Float32Array.
 ...
 */
export class BigInt64Float32HashMap {
  ...
  ): BigInt64Float32HashMap {
    checkExpectedSize(n);
    const onDuplicate = opts?.onDuplicate ?? "error";
    const map = new BigInt64Float32HashMap(hashCapacityFor(n));
  ...
  static bulkLoad(
    ...
    const map =
      hint !== undefined ? new BigInt64Float32HashMap(hashCapacityFor(hint)) : new BigInt64Float32HashMap();
    let i = 0;
    for (const [key, value] of pairs) {
