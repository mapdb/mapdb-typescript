...
import { Float64Int32HashMap } from "./float64-int32-hash-map.js";

/**
 * Immutable Float64→Int32 hash map backed by Float64Array and Int32Array.
 ...
 */
export class ImmutableFloat64Int32HashMap {
  ...
  static of(entries: [number, number][]): ImmutableFloat64Int32HashMap {
    const map = new Float64Int32HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableFloat64Int32HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Float64Int32HashMap): ImmutableFloat64Int32HashMap {
    ...
    return new ImmutableFloat64Int32HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Float64Int32HashMap {
    const result = new Float64Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Float64Int32HashMap {
    const result = new Float64Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
