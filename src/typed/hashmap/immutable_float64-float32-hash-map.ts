...
import { Float64Float32HashMap } from "./float64-float32-hash-map.js";

/**
 * Immutable Float64→Float32 hash map backed by Float64Array and Float32Array.
 ...
 */
export class ImmutableFloat64Float32HashMap {
  ...
  static of(entries: [number, number][]): ImmutableFloat64Float32HashMap {
    const map = new Float64Float32HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableFloat64Float32HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Float64Float32HashMap): ImmutableFloat64Float32HashMap {
    ...
    return new ImmutableFloat64Float32HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Float64Float32HashMap {
  ...
  reject(predicate: (key: number, value: number) => boolean): Float64Float32HashMap {
    const result = new Float64Float32HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
  ...
  toMutable(): Float64Float32HashMap {
    const result = new Float64Float32HashMap();
    for (let i = 0; i < this.capacity; i++) {
