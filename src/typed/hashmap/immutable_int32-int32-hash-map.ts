...
/**
 * Immutable Int32→Int32 hash map backed by Int32Array and Int32Array.
 ...
 */
export class ImmutableInt32Int32HashMap {
  ...
  static of(entries: [number, number][]): ImmutableInt32Int32HashMap {
    const map = new Int32Int32HashMap(entries.length);
    for (const [k, v] of entries) {
      map.set(k, v);
    }
    return ImmutableInt32Int32HashMap.fromMutable(map);
  }

  /** Creates an immutable copy from a mutable map (defensive copy). */
  static fromMutable(mutable: Int32Int32HashMap): ImmutableInt32Int32HashMap {
    ...
    return new ImmutableInt32Int32HashMap(keys, values, occupied, size, c);
  }
  ...
  *valuesIter(): Generator<number> {
  ...
  select(predicate: (key: number, value: number) => boolean): Int32Int32HashMap {
    const result = new Int32Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
  ...
  reject(predicate: (key: number, value: number) => boolean): Int32Int32HashMap {
    const result = new Int32Int32HashMap();
    for (let i = 0; i < this.capacity; i++) {
      if (this.occupied[i] && !predicate(this.keys[i], this.values[i])) {
        result.set(this.keys[i], this.values[i]);
