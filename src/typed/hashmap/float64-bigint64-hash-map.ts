...
/**
 * Float64→BigInt64 hash map backed by Float64Array and BigInt64Array.
 ...
 */
export class Float64BigInt64HashMap {
  ...
  ): Float64BigInt64HashMap {
    checkExpectedSize(n);
    const onDuplicate = opts?.onDuplicate ?? "error";
    const map = new Float64BigInt64HashMap(hashCapacityFor(n));
  ...
  static bulkLoad(
    ...
    const map =
      hint !== undefined ? new Float64BigInt64HashMap(hashCapacityFor(hint)) : new Float64BigInt64HashMap();
    let i = 0;
    for (const [key, value] of pairs) {
