...
import { ImmutableInt16BigInt64HashMap } from "./immutable_int16-bigint64-hash-map.js";
import { Int16BigInt64HashMap } from "./int16-bigint64-hash-map.js";

describe("ImmutableInt16BigInt64HashMap generated", () => {
  it("static of creates immutable map", () => {
    const m = ImmutableInt16BigInt64HashMap.of([[1, 1n], [2, 2n], [3, 3n]]);
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new Int16BigInt64HashMap();
    mutable.set(1, 1n);
    mutable.set(2, 2n);
    const imm = ImmutableInt16BigInt64HashMap.fromMutable(mutable);
    mutable.set(3, 3n);
  ...
  it("get and has", () => {
    const m = ImmutableInt16BigInt64HashMap.of([[1, 1n], [2, 2n]]);
    expect(m.get(1)).toBe(1n);
  ...
  it("getOrDefault", () => {
    const m = ImmutableInt16BigInt64HashMap.of([[1, 1n]]);
    expect(m.getOrDefault(1, 3n)).toBe(1n);
    expect(m.getOrDefault(99, 3n)).toBe(3n);
  });

  it("size and isEmpty", () => {
    const empty = ImmutableInt16BigInt64HashMap.of([]);
    expect(empty.size).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableInt16BigInt64HashMap.of([[1, 1n]]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableInt16BigInt64HashMap.of([[1, 1n], [2, 2n]]);
