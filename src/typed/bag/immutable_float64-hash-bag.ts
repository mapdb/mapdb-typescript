...
export class ImmutableFloat64HashBag {
  ...
  static of(values: number[]): ImmutableFloat64HashBag {
    ...
    return new ImmutableFloat64HashBag(counts, size);
  }

  /** Creates an immutable copy from a mutable bag (defensive copy). */
  static fromMutable(mutable: Float64HashBag): ImmutableFloat64HashBag {
    const counts = new Map<number, number>();
    let size = 0;
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  select(predicate: (value: number) => boolean): Float64HashBag {
    const result = new Float64HashBag();
    for (const [value, count] of this.counts) {
  ...
  reject(predicate: (value: number) => boolean): Float64HashBag {
    const result = new Float64HashBag();
    for (const [value, count] of this.counts) {
      if (!predicate(value)) result.addOccurrences(value, count);
    }
  ...
  /** Returns a Float64Array with all items, each repeated by occurrence count. */
