...
describe("ImmutableFloat64HashBag generated", () => {
  it("static of creates immutable bag", () => {
    const b = ImmutableFloat64HashBag.of([1, 1, 2]);
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new Float64HashBag();
    mutable.add(1);
    mutable.add(1);
    mutable.add(2);
    const imm = ImmutableFloat64HashBag.fromMutable(mutable);
    mutable.add(3);
  ...
  it("size, sizeDistinct, isEmpty", () => {
    const empty = ImmutableFloat64HashBag.of([]);
    expect(empty.size).toBe(0);
    expect(empty.sizeDistinct()).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableFloat64HashBag.of([1, 1]);
  ...
  it("select returns MUTABLE", () => {
    const b = ImmutableFloat64HashBag.of([1, 2, 3]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableFloat64HashBag.of([1, 1, 2]);
  ...
  it("toString", () => {
    const b = ImmutableFloat64HashBag.of([1]);
  ...
  it("defensive copy - modifying source array doesn't affect immutable", () => {
    const source = [1, 2, 3];
    const b = ImmutableFloat64HashBag.of(source);
    source[0] = 99 as any;
    expect(b.has(1)).toBe(true);
    expect(b.has(99)).toBe(false);
  });
