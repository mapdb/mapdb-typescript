...
import { ImmutableInt16HashBag } from "./immutable_int16-hash-bag.js";
import { Int16HashBag } from "./int16-hash-bag.js";

describe("ImmutableInt16HashBag generated", () => {
  it("static of creates immutable bag", () => {
    const b = ImmutableInt16HashBag.of([1, 1, 2]);
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new Int16HashBag();
    mutable.add(1);
    mutable.add(1);
    mutable.add(2);
    const imm = ImmutableInt16HashBag.fromMutable(mutable);
    mutable.add(3);
  ...
  it("size, sizeDistinct, isEmpty", () => {
    const empty = ImmutableInt16HashBag.of([]);
    expect(empty.size).toBe(0);
    expect(empty.sizeDistinct()).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableInt16HashBag.of([1, 1]);
  ...
  it("select returns MUTABLE", () => {
    const b = ImmutableInt16HashBag.of([1, 2, 3]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableInt16HashBag.of([1, 1, 2]);
  ...
  it("toString", () => {
    const b = ImmutableInt16HashBag.of([1]);
  ...
  it("defensive copy - modifying source array doesn't affect immutable", () => {
    const source = [1, 2, 3];
    const b = ImmutableInt16HashBag.of(source);
    source[0] = 99 as any;
    expect(b.has(1)).toBe(true);
    expect(b.has(99)).toBe(false);
  });
