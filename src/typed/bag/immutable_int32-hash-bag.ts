...
export class ImmutableInt32HashBag {
  ...
  static of(values: number[]): ImmutableInt32HashBag {
    ...
    return new ImmutableInt32HashBag(counts, size);
  }

  /** Creates an immutable copy from a mutable bag (defensive copy). */
  static fromMutable(mutable: Int32HashBag): ImmutableInt32HashBag {
    const counts = new Map<number, number>();
    let size = 0;
  ...
  *[Symbol.iterator](): IterableIterator<number> {
  ...
  select(predicate: (value: number) => boolean): Int32HashBag {
    const result = new Int32HashBag();
    for (const [value, count] of this.counts) {
  ...
  reject(predicate: (value: number) => boolean): Int32HashBag {
    const result = new Int32HashBag();
    for (const [value, count] of this.counts) {
      if (!predicate(value)) result.addOccurrences(value, count);
    }
  ...
  /** Returns a Int32Array with all items, each repeated by occurrence count. */
  toArray(): Int32Array {
