...
import { ImmutableInt32HashBag } from "./immutable_int32-hash-bag.js";
import { Int32HashBag } from "./int32-hash-bag.js";

describe("ImmutableInt32HashBag generated", () => {
  it("static of creates immutable bag", () => {
    const b = ImmutableInt32HashBag.of([1, 1, 2]);
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new Int32HashBag();
    mutable.add(1);
    mutable.add(1);
    mutable.add(2);
    const imm = ImmutableInt32HashBag.fromMutable(mutable);
    mutable.add(3);
  ...
  it("size, sizeDistinct, isEmpty", () => {
    const empty = ImmutableInt32HashBag.of([]);
    expect(empty.size).toBe(0);
    expect(empty.sizeDistinct()).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableInt32HashBag.of([1, 1]);
  ...
  it("select returns MUTABLE", () => {
    const b = ImmutableInt32HashBag.of([1, 2, 3]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableInt32HashBag.of([1, 1, 2]);
  ...
  it("toString", () => {
    const b = ImmutableInt32HashBag.of([1]);
  ...
  it("defensive copy - modifying source array doesn't affect immutable", () => {
    const source = [1, 2, 3];
    const b = ImmutableInt32HashBag.of(source);
    source[0] = 99 as any;
    expect(b.has(1)).toBe(true);
    expect(b.has(99)).toBe(false);
  });
