...
export class ImmutableInt16HashBag {
  ...
  static of(values: number[]): ImmutableInt16HashBag {
    ...
    return new ImmutableInt16HashBag(counts, size);
  }

  /** Creates an immutable copy from a mutable bag (defensive copy). */
  static fromMutable(mutable: Int16HashBag): ImmutableInt16HashBag {
    const counts = new Map<number, number>();
    let size = 0;
