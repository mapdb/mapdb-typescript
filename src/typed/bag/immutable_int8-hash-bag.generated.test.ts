...
describe("ImmutableInt8HashBag generated", () => {
  it("static of creates immutable bag", () => {
    const b = ImmutableInt8HashBag.of([1, 1, 2]);
  ...
  it("fromMutable creates defensive copy", () => {
    const mutable = new Int8HashBag();
    mutable.add(1);
    mutable.add(1);
    mutable.add(2);
    const imm = ImmutableInt8HashBag.fromMutable(mutable);
    mutable.add(3);
  ...
  it("size, sizeDistinct, isEmpty", () => {
    const empty = ImmutableInt8HashBag.of([]);
    expect(empty.size).toBe(0);
    expect(empty.sizeDistinct()).toBe(0);
    expect(empty.isEmpty()).toBe(true);
    const nonEmpty = ImmutableInt8HashBag.of([1, 1]);
  ...
  it("select returns MUTABLE", () => {
    const b = ImmutableInt8HashBag.of([1, 2, 3]);
  ...
  it("toMutable round-trip", () => {
    const original = ImmutableInt8HashBag.of([1, 1, 2]);
  ...
  it("toString", () => {
    const b = ImmutableInt8HashBag.of([1]);
  ...
  it("defensive copy - modifying source array doesn't affect immutable", () => {
    const source = [1, 2, 3];
    const b = ImmutableInt8HashBag.of(source);
    source[0] = 99 as any;
    expect(b.has(1)).toBe(true);
    expect(b.has(99)).toBe(false);
  });
