...
/**
 * Open-addressing hash map with number keys and number values.
 * Uses a bitmap (occupied array) to track which slots contain data.
 */
export class NumberNumberHashMap implements MapDbMutableMap<number, number> {
  private keys: number[];
  ...
  static bulkLoadExact(
    ...
    const m = new NumberNumberHashMap(hashCapacityFor(n));
  ...
  static bulkLoad(
    ...
    const m =
      hint !== undefined ? new NumberNumberHashMap(hashCapacityFor(hint)) : new NumberNumberHashMap();
    let i = 0;
    for (const [key, value] of pairs) {
  ...
  *valuesIter(): Generator<number> {
  ...
  private resize(): void {
    ...
    this.keys = new Array<number>(newCap).fill(0);
