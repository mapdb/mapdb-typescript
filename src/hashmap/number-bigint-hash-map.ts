...
/**
 * Open-addressing hash map with number keys and bigint values.
 * Uses a bitmap (occupied array) to track which slots contain data.
 */
export class NumberBigIntHashMap implements MapDbMutableMap<number, bigint> {
  private keys: number[];
  ...
  static bulkLoadExact(
    ...
    const m = new NumberBigIntHashMap(hashCapacityFor(n));
  ...
  static bulkLoad(
    ...
    const m =
      hint !== undefined ? new NumberBigIntHashMap(hashCapacityFor(hint)) : new NumberBigIntHashMap();
    let i = 0;
    for (const [key, value] of pairs) {
