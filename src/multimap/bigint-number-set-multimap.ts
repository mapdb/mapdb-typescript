...
export class BigIntNumberSetMultimap {
  ...
  constructor() {
    this._map = new Map<bigint, number[]>();
    this._totalSize = 0;
  }
  ...
  static fromSortedKeys(
    ...
    return BigIntNumberSetMultimap.fromSorted(sortedPairs);
  }
  ...
  containsKeyValue(key: bigint, value: number): boolean {
    const list = this._map.get(key);
  ...
  select(
    ...
    const result = new BigIntNumberSetMultimap();
    this._map.forEach((list, key) => {
      for (let i = 0; i < list.length; i++) {
  ...
  equals(other: BigIntNumberSetMultimap): boolean {
...
/**
 * Streaming builder for a {@link BigIntNumberSetMultimap} (the data pump's Sink form). Buffer
 ...
 */
export class BigIntNumberSetMultimapSink {
  private mm: BigIntNumberSetMultimap = new BigIntNumberSetMultimap();
  ...
  create(): BigIntNumberSetMultimap {
    if (this.poisoned) throw new Error("sink is poisoned after a prior error");
    if (this.done) throw new Error("sink already created");
    this.done = true;
    return this.mm;
