...
/**
 * A multimap that maps bigint keys to lists of bigint values.
 * Backed by a JavaScript Map from key to array of values.
 */
export class BigIntBigIntListMultimap {
  ...
  static of(): BigIntBigIntListMultimap {
    return new BigIntBigIntListMultimap();
  }
  ...
  ): BigIntBigIntListMultimap {
    const mm = new BigIntBigIntListMultimap();
  ...
  static fromSortedKeys(
    ...
    return BigIntBigIntListMultimap.fromSorted(sortedPairs);
  }
  ...
  select(
    ...
    const result = new BigIntBigIntListMultimap();
    this._map.forEach((list, key) => {
      for (let i = 0; i < list.length; i++) {
  ...
  equals(other: BigIntBigIntListMultimap): boolean {
...
/**
 * Streaming builder for a {@link BigIntBigIntListMultimap} (the data pump's Sink form). Buffer
 ...
 */
export class BigIntBigIntListMultimapSink {
  private mm: BigIntBigIntListMultimap = new BigIntBigIntListMultimap();
  ...
  create(): BigIntBigIntListMultimap {
    if (this.poisoned) throw new Error("sink is poisoned after a prior error");
    if (this.done) throw new Error("sink already created");
    this.done = true;
    return this.mm;
