...
export class BigIntBigIntTreeMap implements MapDbMutableMap<bigint, bigint> {
  ...
  static fromSorted(
    ...
    const sink = new BigIntBigIntTreeMapSink(opts);
  ...
  static buildFromSortedBuffer(
    ...
    const map = new BigIntBigIntTreeMap();
    map.root = buildRedBlack<BigIntBigIntTreeMapNode>(keys.length, (j) => ({
      key: keys[j],
      value: values[j],
      left: null,
      right: null,
...
/**
 * Streaming builder for a {@link BigIntBigIntTreeMap} from ascending-sorted pairs (the data
 ...
 * once-only.
 */
export class BigIntBigIntTreeMapSink {
  ...
  create(): BigIntBigIntTreeMap {
    if (this.poisoned) throw new Error("sink is poisoned after a prior error");
    if (this.done) throw new Error("sink already created");
    this.done = true;
    return BigIntBigIntTreeMap.buildFromSortedBuffer(this.keys, this.values);
  }
}
